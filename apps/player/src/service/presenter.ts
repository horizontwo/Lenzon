...
export class Presenter {
  ...
  /**
   * Branding layer — sits above canvas/DOM/3D for intro/outro Lottie playback.
   * Sibling element so transitions on stageRoot leave it alone.
   */
  readonly chromeRoot: HTMLElement | null;
  ...
  private lottie: LottieOverlay | null = null;
  ...
  showTextBox(
    ...
    const box = new TextBox(opts.text, opts.style, opts.maxWidth);
  ...
  present(spec: { template: string; content: TemplateContent }): TemplateHandle {
    ...
    if (!template) {
      throw new Error(`lenzon: unknown template "${spec.template}"`);
    }
  ...
  /**
   * Mount a Lottie animation on the chrome layer. `onComplete` fires when
   * the animation finishes (or immediately if the package/asset is missing,
   * so the caller's state machine never stalls). No-op if no chromeRoot
   * was provided to the Presenter.
   */
  showLottie(src: string, onComplete: () => void): void {
    if (!this.chromeRoot) {
      onComplete();
      return;
    }
    this.hideLottie();
    this.lottie = new LottieOverlay(this.chromeRoot, src, onComplete);
  }
