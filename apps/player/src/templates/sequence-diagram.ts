 ...
 * is on the stack.
 *
 * lenzon/ui v0.1 styling: the actor header chrome, labels, and
 * activation bars all consume --cs-* tokens (ink + cool + warm accents).
 * The layout math (HEADER_H, ROW_H, row geometry) is intentionally
 * unchanged — TEMPLATE-SPEC §9 #4 flags responsive sizing as a known
 * issue but the migration spec says keep the math and swap only visual
 * chrome. Responsive sizing remains on the backlog.
 *
 * Slot schema:
 *   title?:    string
...
export const sequenceDiagramTemplate: Template = {
  ...
  render(presenter, contentIn) {
    ...
    if (title) {
      ...
      h.className = 'cs-title cs-title--m sb-seq-title';
      h.textContent = title;
    ...
    actors.forEach((actor, i) => {
      ...
      inner.className = 'cs-plate cs-plate--sunken sb-seq-actor-inner';
      inner.textContent = actor.label;
    ...
    // Arrowhead marker defs. SVG fill accepts `var(--cs-*)` directly.
    const defs = document.createElementNS(svgNS, 'defs');
    defs.innerHTML = `
      <marker id="sb-seq-arrow-req" viewBox="0 0 10 10" refX="9" refY="5"
              markerWidth="6" markerHeight="6" orient="auto-start-reverse">
        <path d="M0,0 L10,5 L0,10 Z" fill="var(--cs-ink-1)"/>
      </marker>
    ...
    steps.forEach((step, i) => {
      ...
      // Label — anchored at midpoint of the arrow. Font size lives in CSS
      // (.sb-seq-label) so the design system's --cs-fs-* scale governs it.
        ...
        if (path && path.getTotalLength) {
          try {
            ...
            if (len > 0) {
              ...
              // After the draw-in completes, drop the dash machinery so
              // the path renders as a clean solid line. getTotalLength()
              // can under-report on paths with non-scaling-stroke,
              // leaving a stub at the head end if we leave the dasharray
              // in place.
    ...
    // --- Handle --------------------------------------------------------
    // Accent budget = 1. Track the currently-pulsed step and actor so a
    // new emphasize call clears the prior one before lighting the next.
    let activeStep: number | null = null;
    let activeStepTimer: number | null = null;
    let activeActor: HTMLElement | null = null;
    let activeActorTimer: number | null = null;

    const clearActiveStep = () => {
      if (activeStep == null) return;
      stepPaths[activeStep]?.classList.remove('sb-seq-pulse');
      stepLabels[activeStep]?.classList.remove('sb-seq-pulse');
      stepActivations[activeStep]?.classList.remove('sb-seq-pulse');
      if (activeStepTimer != null) window.clearTimeout(activeStepTimer);
      activeStep = null;
      activeStepTimer = null;
    };
    const clearActiveActor = () => {
      if (!activeActor) return;
      activeActor.classList.remove('sb-seq-actor-active');
      if (activeActorTimer != null) window.clearTimeout(activeActorTimer);
      activeActor = null;
      activeActorTimer = null;
    };
    ...
    const handle: TemplateHandle = {
      ...
      emphasize: (target) => {
        ...
        if (Number.isFinite(idx) && stepRows[idx]) {
          ...
          activeStep = idx;
          activeStepTimer = window.setTimeout(() => {
        ...
        if (actor) {
          ...
          activeActor = actor;
          activeActorTimer = window.setTimeout(() => {
            actor.classList.remove('sb-seq-actor-active');
            activeActor = null;
            activeActorTimer = null;
          }, 1400);
        }
