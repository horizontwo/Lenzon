 ...
 * including the lenzon hero embed (which uses a CSS transform that
 * defeats canvas/WebGL backbuffer sizing).
 *
 * lenzon/ui v0.1 styling: node rects consume --cs-glass-* tokens
 * directly in their fill/stroke attributes (SVG resolves var() once the
 * element is attached). Labels sit in a foreignObject and read as
 * cs-plate-header-style text (display-italic label, mono ui color).
 * Group colors remain resolved via resolveColor / PALETTE_DEFAULTS so
 * Producer-emitted `palette.*` aliases keep working; the resolved color
 * drives the left accent stripe on each node.
 *
...
const NODE_W = 220;     // node card width
const NODE_H = 80;      // node card height
...
export const flowDiagramTemplate: Template = {
  ...
  slots: {
    nodes: '{ id, label, group? }[] — nodes in the graph',
  ...
  demo: {
    ...
    content: {
      nodes: [
        { id: 'client', label: 'Browser', group: 'frontend' },
  ...
  render(presenter, contentIn) {
    ...
    // aspect. Use clientWidth/clientHeight, not getBoundingClientRect —
    // CSS transforms on ancestor hosts (e.g. the hero embed) distort
    // rect values while clientWidth stays in the design coord frame. See
    // docs/TEMPLATE-SPEC.md §9b.
    const viewW = presenter.domRoot.clientWidth || 1280;
    const viewH = presenter.domRoot.clientHeight || 720;
    ...
    // Arrowhead marker. SVG fill reads var(--cs-*) directly once attached.
    const defs = document.createElementNS(SVG_NS, 'defs');
    defs.innerHTML = `
      <marker id="sb-flow-arrow" viewBox="0 0 10 10" refX="9" refY="5"
              markerWidth="6" markerHeight="6" orient="auto-start-reverse">
        <path d="M0,0 L10,5 L0,10 Z" fill="var(--cs-ink-3)"/>
      </marker>
    `;
    ...
    trimmedNodes.forEach((n, i) => {
      ...
      const color = (n.group && groupColor.get(n.group)) || 'var(--cs-ink-4)';
      const g = buildNode(pos.x, pos.y, n.label, color);
    ...
    // Producer sometimes hands us labels longer than the card can hold
    // (e.g. "performWorkUntilDeadline"). Step the font down until the
    // text no longer overflows, or we hit the floor and let CSS ellipsis
    // take over. Must run after attach so foreignObject children measure.
    const FONT_STEPS = [20, 18, 16, 14];
    const rafId = requestAnimationFrame(() => {
      const labels = wrapper.querySelectorAll<HTMLDivElement>('.sb-flow-node-label');
      labels.forEach((div) => {
        for (const size of FONT_STEPS) {
          div.style.fontSize = `${size}px`;
          if (div.scrollWidth <= div.clientWidth) break;
        }
      });
    });
    ...
    // Accent budget = 1. Track the currently-pulsed node so a new
    // emphasize call clears the prior one first.
    let activeRect: SVGRectElement | null = null;
    let activeOrigStroke: string | null = null;
    let activeTimer: number | null = null;
    const clearActive = () => {
      if (!activeRect) return;
      activeRect.setAttribute('stroke', activeOrigStroke ?? 'var(--cs-glass-border)');
      activeRect.setAttribute('stroke-width', '1.5');
      if (activeTimer != null) window.clearTimeout(activeTimer);
      activeRect = null;
      activeOrigStroke = null;
      activeTimer = null;
    };
    ...
    const handle: TemplateHandle = {
      ...
      emphasize: (target) => {
        ...
        clearActive();
        const orig = rect.getAttribute('stroke') ?? 'var(--cs-glass-border)';
        rect.setAttribute('stroke', 'var(--cs-accent-warm)');
        rect.setAttribute('stroke-width', '3');
        activeRect = rect;
        activeOrigStroke = orig;
...
/**
 * Build a node group: rounded glass-plate rect + foreignObject label.
 * Fill/stroke read cs tokens so the node picks up the same glass surface
 * as the DOM .cs-plate primitives. Centred on (cx, cy) in viewBox units.
 */
function buildNode(
  cx: number,
  cy: number,
  ...
  // Background card — glass bg + glass border. The design-system plate
  // primitives ship these same tokens; rendering them via SVG rect
  // attributes keeps the node crisp at any viewBox scale.
  ...
  bg.setAttribute('fill', 'var(--cs-glass-bg)');
  bg.setAttribute('stroke', 'var(--cs-glass-border)');
  bg.setAttribute('stroke-width', '1.5');
  g.appendChild(bg);

  // Left accent stripe in the group color (producer-driven).
  ...
  // Label via foreignObject — native text rendering, ellipsis, font
  // fallback. Inset from the stripe. Inline styles reference cs tokens
  // so the label stays in sync with the ink scale.
...
function buildEdge(
  ...
  path.setAttribute('stroke', 'var(--cs-ink-3)');
  ...
  if (label) {
    ...
    bg.setAttribute('fill', 'var(--cs-glass-sunken-bg)');
    g.appendChild(bg);
    g.appendChild(text);
  }
...
/**
 * Longest-path rank per node, used by the layered layout to place each
 * node in a "column" (or "row", depending on orientation).
 *
 * Why cycle detection matters: the naive longest-path relaxation diverges
 * on directed cycles — each outer pass lets every node on the cycle pump
 * its rank by one, and downstream nodes inherit the inflation. The result
 * is ranks of 10+ for a graph that should lay out in 3 columns, which
 * manifests as huge horizontal gaps and disproportionately long edges.
 * We break feedback edges (those that point to an ancestor) before
 * relaxing, so ranks stay bounded by `nodes.length - 1`.
 */
function computeRanks(nodes: NodeSpec[], edges: EdgeSpec[]): number[] {
  ...
  for (const e of edges) {
    ...
    if (fi != null && ti != null) outgoing[fi].push(ti);
  }

  // DFS to find a forward (acyclic) edge set. Edges that close a cycle
  // (target is currently on the DFS stack) are dropped for ranking.
  const WHITE = 0;
  const GRAY = 1;
  const BLACK = 2;
  const color = new Array(nodes.length).fill(WHITE);
  const forward: number[][] = Array.from({ length: nodes.length }, () => []);

  const visit = (u: number) => {
    color[u] = GRAY;
    for (const v of outgoing[u]) {
      if (color[v] === GRAY) continue; // back-edge → cycle; drop it
      forward[u].push(v);
      if (color[v] === WHITE) visit(v);
    }
    color[u] = BLACK;
  };
  for (let i = 0; i < nodes.length; i++) if (color[i] === WHITE) visit(i);

  // Relax longest paths over the cycle-free edge set.
  const incoming: number[][] = Array.from({ length: nodes.length }, () => []);
  for (let u = 0; u < nodes.length; u++) {
    for (const v of forward[u]) incoming[v].push(u);
  }
