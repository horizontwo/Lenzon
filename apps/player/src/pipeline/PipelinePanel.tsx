...
export function PipelinePanel({ onPlayScript, canPlay }: PipelinePanelProps) {
  ...
  const runAnalyze = async () => {
    ...
    try {
      const { id } = await runTriage(repoUrl);
      const record = await pollTriage(id);
      if (record.status !== 'ready' || !record.data) {
        throw new Error(record.error ?? 'triage failed');
      }
      setTriageReport(record.data);
      setTriageSessionId(record.sessionId);
      setStage('idle');
      setAnalyzeProgress('');
  ...
  const runDeepAnalysis = async (mode: AnalysisMode) => {
    ...
    const sessionId = triageSessionId;
    ...
    try {
      const { id } = await startAnalyze(repoUrl, mode, report, sessionId);
